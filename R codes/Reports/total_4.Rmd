...
set.seed(1)
sample <- sample(c(TRUE, FALSE), nrow(daydata), replace=TRUE, prob=c(0.7,0.3))
...
m= lm(cnt ~ temp+ I(temp^2)+ I(temp^3) + windspeed +as.factor(weathersit) +as.factor(yr))
summary(m)
```
...
```{r}
m1 <- gls(cnt ~ temp+ I(temp^3) + windspeed + as.factor(season) +as.factor(weathersit) +as.factor(yr),correlation=corAR1(form=~instant),data=train,method="ML")
summary(m1)
```
